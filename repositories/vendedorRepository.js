...
export default class VendedorRepository{
    ...
    async cadastrarVendedor(novoVendedor){
        const sql = "insert into tb_vendedor (vendedor_nome, vendedor_email) values (?, ?)"
        const values = [novoVendedor.vendedor_nome, novoVendedor.vendedor_email];
    ...
    async listarVendedores(){
        const sql = "select * tb_vendedor where ven_ativo = 1";
    ...
    async procurarVendedor(vendedor_id){
        ...
        const sql = "select * from tb_vendedor where vendedor_email = ? and vendedor_nome = ?";
